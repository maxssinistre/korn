...
#modified this for cygwin
     if [ $(find ./ ${ELAPSE} -size ${BIG} -print | grep -v tbz | egrep -i "(${EXT})" | DOUBLES ${SINGLE} | wc -l ) -ge 1 ] ; then 
...
#modified this for cygwin
     ...
     if [ $(find ./ ${ELAPSE} -size ${BIG} -print | grep -v tbz | egrep -i "(${EXT})" | DOUBLES | wc -l ) -ge 1 ] ; then
...
#		find ./ ${ELAPSE} -size ${BIG} \( -iname "*.w*" -o -iname "*.video*" -o -iname "*.m*" -o -iname "*.avi" -o -iname "*.flv*" \) -printf '%p\n' -exec ${MPLAY_BIN} ${OPTIONS} -speed ${SPEED} -loop ${LOOP} ${SOUNDS} -cache 8192 -xy ${FBMODE} -zoom -vo fbdev2 -idx -fs -osdlevel 3 {} \;
		...
		find ./ ${ELAPSE} -size ${BIG} \( -iname "*.w*" -o -iname "*.video*" -o -iname "*.m*" -o -iname "*.avi" -o -iname "*.flv*" \) -printf '%p\n' -exec ${MPLAY_BIN} ${OPTIONS} -speed ${SPEED} -loop ${LOOP} ${SOUNDS} -cache 8192 -x ${FBX} -y ${FBY} -zoom -vo fbdev2 -idx -fs -osdlevel 3 {} \;
...
#		find ./ ${ELAPSE} -size ${BIG} -iname "*${EXT}*" -exec ${MPLAY_BIN} ${OPTIONS} -speed ${SPEED} -loop ${LOOP} ${SOUNDS} -cache 8192 -xy ${FBMODE} -zoom -vo fbdev2 -idx -fs -osdlevel 3 {} \;

		find ./ ${ELAPSE} -size ${BIG} -iname "*${EXT}*" -exec ${MPLAY_BIN} ${OPTIONS} -speed ${SPEED} -loop ${LOOP} ${SOUNDS} -cache 8192 -x ${FBX} -y ${FBY} -zoom -vo fbdev2 -idx -fs -osdlevel 3 {} \;
...
while getopts :t:ivfc2nlxD:S:md:e:sbp:L:P:o:B: option
do
      case $option in
                t) NoMt="${OPTARG}" ;;
                v) XV=1 ;;
