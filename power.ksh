...
wget http://www.t-nation.com/tmagnum/powerfulImages.jsp?pageNo=0

####get foldername and make foldername move files to working folder

DIRECT=$(grep -i title *powerfulImages.jsp* |head -1 |sed 's/<[a-zA-Z\/][^>]*>//g' | sed 's#TMAGNUM \FORUMS\ -\ ##g' | sed 's#TESTOSTERONE\ NATION\ -##g' | cut -d ":" -f2 | sed 's#\ ##g' | tr -d [:punct:])
...
#count=$(cat *powerfulImages.jsp* | awk -F\<a '/Last/ { print $NF }' | grep -o 'pageNo=[^"Last]*' | awk '! a[$0]++' | sed 's#pageNo=##g')
count=5
#AMOUNT=57000
      	
		while [ ${count} -gt 0 ]
		do
			wget http://www.t-nation.com/tmagnum/powerfulImages.jsp?pageNo=$count
			let count=$count-1
...
####cleanup files
tar -cvzf archive_$RANDOM.tgz *.jsp* $FILE*
rm -rf *.jsp*
rm -f $FILE*
