...
NUM=$(mplayer -dvd-device /dev/sr0 dvd://1 -vo null -ao null -frames 0 -v 2>&1 | grep ^There |grep titles|grep -o [0-9]*[0-9])

count=$((${NUM}/1))

while test $count -gt 0
do
        mencoder -dvd-device /dev/sr0 dvd://$count -idx -o dvd_extract_$(date +%Y%m%d%H%M%S).avi -alang en -ovc lavc -oac copy -lavcopts vcodec=mpeg4:vbitrate=$BIT
